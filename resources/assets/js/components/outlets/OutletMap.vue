...
<div id="outlet_map">
  <div class="box box-danger box-solid">
    <div class="box-header with-border">
      <div class="row">
        <div class="col-sm-8">
          <h3 class="box-title">Outlet Map</h3>
        </div>
        <div class="col-sm-4">
...
import { outletUrl } from '../../globalConfig.js';
export default {
  data: () => ({
    location: {
      lat: 10.0,
  ...
  methods: {
    ...
    setLocation() {
      axios.put(outletUrl + '/' + this.outlet.id + '/location', {
        lat: this.location.lat,
        lng: this.location.lng,
        location: this.location.location
      }).then((resp) => {
        if (resp.status === 200) {
          this.$store.commit('currentOutlet', resp.data.data)
