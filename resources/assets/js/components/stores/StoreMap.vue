...
<div id="store_map">
  <div class="box box-danger box-solid">
    <div class="box-header with-border">
      <div class="row">
        <div class="col-sm-8">
          <h3 class="box-title">Store Map</h3>
        </div>
        <div class="col-sm-4">
...
import { storeUrl } from '../../globalConfig.js';
export default {
  data: () => ({
    location: {
      lat: 10.0,
  ...
  methods: {
    ...
    setLocation() {
      axios.put(storeUrl + '/' + this.store.id + '/location', {
        lat: this.location.lat,
        lng: this.location.lng,
        location: this.location.location
      }).then((resp) => {
        if (resp.status === 200) {
          this.$store.commit('currentStore', resp.data.data)
    ...
    setInitLoc: _.debounce(function() {
      if (this.store) {
        this.location.lat = this.store.lat
        this.location.lng = this.store.lng
      }
    }, 1000)
  },
  computed: {
